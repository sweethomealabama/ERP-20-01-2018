...
Calendar.prototype.toString = function() {
	...
	str += this.Obj;
	str += '.showMenu(null);" onmouseout="';
	str += this.Obj;
	str += '.hideMenu();"></div></div>';
	return str;
};
Calendar.prototype.getStyle = function() {
	...
	str += this.style.bodyBackColor;
	str += ';border:1px solid ';
	str += this.style.borderColor;
	str +=';left:0px;top:0px;z-index:9999;}\n';
	str += '.cdrHeader{background-color:';
	str += this.style.headerBackColor;
	str += ';width:140px;height:22px;font-size:12px;color:';
	str += this.style.headerFontColor + ';}\n';
	str += '.cdrWatermark{position:absolute;left:0px;top:55px;width:140px;font-family: 黑体;font-size:70px;color:';
	str += this.style.watermarkColor;
	str +=';z-index:1;text-align:center;}\n';
	str += '.cdrBodyBar{background-color:'; 
	str += this.style.bodyBarBackColor;
	str += ';font-size:12px;color:'; 
	str += this.style.bodyBarFontColor;
	str += ';width:140px;height:20px;}\n';
	str += '.cdrBody{width:140px;height:122px!important; height /**/:110px;font-size:12px;cursor:pointer;color:';
	str += this.style.bodyFontColor + ';}\n';
...
Calendar.prototype.getBody = function() {
	...
	for (i = 0; i < 6; i++) {
		...
		var j = 0;
		for (j = 0; j < 7; j++) {
			str = str + '<td Author="alin" class="dayOut" id="cdrDay' + (n)
					+ '" width="13%"></td>\n';
			n = n + 1;
		}
		str = str + '</tr>';
	...
	str = str + '</table>\n';
	str = str
			+ '<table Author="alin" class="cdrBodyBar" cellSpacing="2" cellPadding="0"><tr align="center" Author="alin"><td Author="alin" style="cursor:pointer;" onclick="'
			+ this.Obj + '.getToday();">今天：'
...
Calendar.prototype.bindDate = function(date) {
	...
	// if (_previMonth == 1)_previDay
	// =((_previYear%4==0)&&(_previYear%100!=0)||(_previYear%400==0))?29:28;
	...
	var _previDate, _nextDate, _curDate, _day, i = 0, temp = new Date(_year,
			_month, 1).getDay()
...
function getPosition(e) {
	 ...
	 * (e.currentStyle?(parseInt(e.currentStyle.borderLeftWidth)).NaN0():0); top +=
	 * e.offsetTop +
	 * (e.currentStyle?(parseInt(e.currentStyle.borderTopWidth)).NaN0():0); e =
	 * e.offsetParent; }
	 */
